...
Autotuning::Autotuning(double min, double max, int ignore, int dim, int num_opt,
                       int max_iter)
    : Autotuning(min, max, ignore, new CSA(dim, num_opt, max_iter)) {}

Autotuning::Autotuning(double min, double max, int ignore,
                       NumericalOptimizer *optimizer)
    : m_min(min), m_max(max), m_iter(0), m_ignore(ignore + 1), m_cost(0),
      m_runtime(0), p_optimizer(optimizer) {

  printf("Autotuning::Autotuning\n");
