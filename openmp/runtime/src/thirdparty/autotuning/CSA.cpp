...
void CSA::swap_opt_info(unsigned i) {
...
void CSA::reset(unsigned level) {
  ...
  case 0: // Reset only number of iteretions
    ...
    //   std::to_string(level)); break;
  }
}

CSA::CSA(unsigned dim, unsigned num_opt, unsigned max_iter)
    : m_maxCost(0.0), m_bestCost(0.0) {
  // if (dim < 1) {
  //   throw std::invalid_argument("Dimensional Value Invalid! Set dim > 0.");
  // }
  // if (num_opt < 1) {
  //   throw std::invalid_argument("Optmizers Number Invalid! Set num_opt >
  //   0.");
  // }
  // if (max_iter < 1) {
  //   throw std::invalid_argument("Max number of intereration Invalid! Set
  //   max_iter > 0.");
  // }
  ...
  // try {
  this->m_maxIter = static_cast<unsigned>(max_iter / (double)num_opt);
